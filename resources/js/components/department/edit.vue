<script setup>
    ...
    const getDepartment = async () => {
        let response = await axios.get(`/api/edit_department/${props.id}`)
        form.value = response.data.departments
    }
	...
	const onEdit = (id) => {
		...
		axios.post(`/api/update_department/${form.value.id}`,formData).then(function () {
