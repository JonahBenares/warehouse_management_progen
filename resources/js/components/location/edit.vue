...
const router = useRouter()
	...
	const getLocation = async () => {
        let response = await axios.get(`/api/edit_location/${props.id}`)
       form.value = response.data.locations
    }
	...
	axios.post(`/api/update_location/${form.value.id}`,formData).then(function () {
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="form-group">
										...
										<input type="text" class="form-control border" v-model="form.location_name">
									</div>										
								</div>
							</div>
