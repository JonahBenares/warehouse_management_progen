...
const getEmployees = async (page = 1) => {
	const response = await fetch(`/api/get_all_employees?page=${page}&filter=${searchEmployees.value}`);
...
const search = async () => {
	let response = await fetch('/api/search_employees?filter='+searchEmployees.value);
    employees.value = await response.json();
}
</script>
							...
							<div class="flex justify-between pb-2 mt-2 mb-2">
								<div class="flex justify-between">
									<div class="input-group border rounded-xl w-80">
										...
										<input type="text" class="form-control border-0" id="search" placeholder="Type to search..." @keyup="getEmployees()" v-model="searchEmployees">
									</div>
								</div>
								<a href="/employees/new" class="btn btn-sm btn-primary btn-rounded">
							...
							<table class="table table-borsdered table-hover mb-0">
								...
								<tbody>
									<tr v-for="employee in employees.data" :key="employee.id" >
										...
										<td v-if="employee.department">{{ employee.department.department_name }}</td>
										<td v-else></td>
										<td>{{ employee.contact_no }}</td>
										<td>{{ employee.email }}</td>
										<td class="p-1">
											<a @click="onEdit(employee.id)" class="btn btn-xs btn-info btn-rounded text-white">
