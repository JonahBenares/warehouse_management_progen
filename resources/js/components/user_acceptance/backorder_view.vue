<script setup>
	...
	const saveAccepted = () => {
		...
		formData.append('backorder_loop',JSON.stringify(details.value))
		formData.append('eval_date', eval_date.value)
		formData.append('eval_user', eval_user.value)
		formData.append('backorder_head_id', props.id)
		axios.post(`/api/save_backaccepted`, formData).then(function (response) {
			// console.log(response.data)
			if(response.data!='error'){
				success.value='Successfully accepted!'
	...
	const saveRejected = () => {
		...
		// 	}
		// }
		axios.post(`/api/save_backrejected`, formData).then(function (response) {
			if(response.data!='error'){
	...
	const onPrint= (id) => {
		router.push('/user_acceptance/backorder_print/'+id)
	}
	...
	const checkAcceptanceQty = (counter) => {
		...
		checkbox.value.forEach(function (val, index, theArray) {
			if(val!=0){
				...
				rejected_checker.value+=parseFloat(document.getElementById("rejected_qty"+val).value);
	...
	const saveNew = () => {
		...
		formData.append('eval_date', eval_date.value)
		formData.append('eval_user', eval_user.value)
		// formData.append('eval_user_reject', eval_user_reject.value)
		formData.append('backorder_head_id', props.id)
		checkbox.value.forEach(function (val, index, theArray) {
			formData.append('rejected_qty'+index, rejected_qty.value[val])
		...
		if(!Object.values(array).includes(false)){
			...
			axios.post(`/api/save_newbackaccepted`, formData).then(function (response) {
				if(response.data!='error'){
        ...
        <div class="container-fluid">
					...
					<div class="pt-1">	
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb adminx-page-breadcrumb">
								...
								<li class="breadcrumb-item"><a href="/user_acceptance/pending">Backorder User Acceptance</a></li>
			...
			<div class="card p-4 card-main-bg">
							...
							<table class="w-full table-bordersed mt-2 mb-2 ">
								<tr>
									...
									<td class="px-1" width="10%">
										<div class="flex justify-start">
											<span class="text-lg uppercase font-bold text-gray-600 leading-none">{{ head.backorder_date }}</span>
										</div>
										<div class="flex justify-start" >
									...
									<td class="px-1  "  width="15%">
										<div class="flex justify-start">
											<span class="text-md uppercase font-bold text-gray-600 leading-none">{{ (head.dr_no!=null) ? head.dr_no : dr_no }}</span>
										</div>
										<div class="flex justify-start">
									...
									<td class="px-1  " width="2%">
										<span class="flex justify-center text-green-600">
											<CheckCircleIcon fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5" v-if="(head.pcf != null || pcf!=0)"></CheckCircleIcon>
											<span v-else></span>
										</span>
				...
				<div v-for="det in details" class="border-2 border-blue-400 mb-3 rounded ">
							...
							<table class="w-full table-borsdered">
								<tr>
									<td width="2%" rowspan="4" class="align-top p-0 bg-blue-400"> 
										<div class="pt-2 p-1  px-2 text-md text-center bg-blue-400  font-bold pb-5 text-white">{{ x.toString().padStart(2, '0') }}</div>
									</td>
									...
									<td class="pt-2 px-1 text-sm border-b">{{ det.inspected_by }}</td>
								</tr>
								<tr>
									<td class="pl-2 form-label" >Department</td>
									<td class="px-1 text-sm border-b">{{ det.department }}</td>
									<td class="px-1 text-sm" width="3%"></td>
									<td class="form-label">Enduse</td>
									<td class="px-1 text-sm border-b" colspan="2">{{ det.enduse }}</td>
								</tr>
								<tr>
									<td class="pl-2 form-label">Purpose</td>
									<td class="px-1 text-sm " colspan="5">{{ det.purpose }}</td>
								</tr>
								<tr>
					...
					<div class="row">
						<div class="col-lg-12">
							<table class="table w-full table-actions table-bordered table-hodver mb-0 border-t-2">
								<tbody  v-for="(it,index) in det.receive_items.items">
									<tbody :class="(it.eval_flag==1) ? '' : (it.eval_flag==2) ? '!border-2 !border-red-400' : ''">
										<tr class="">
											...
											<td class="font-xxs uppercase font-bold" width="5%">Unit Cost</td>
											<td class="font-xxs uppercase font-bold" width="5%">Currency</td>
											<td class="font-xxs uppercase font-bold" width="5%">BO Qty</td>
											<!-- <td class="font-xxs uppercase font-bold" width="5%">Original BO Qty</td> -->
										...
										<tr>
											...
											<td class="p-1 font-xxss" colspan="2">{{ it.item_description }} {{ (it.pn_no!=null) ? " - "+it.pn_no : '' }}</td>
											...
											<td class="p-1 font-xxss">{{ (it.currency!=null) ? it.currency : '' }}</td>
											<td class="p-1 font-xxss">{{ it.bo_quantity }}</td>
											<!-- <td class="p-1 font-xxss">{{ it.bo_quantity + it.rejected_qty }}</td> -->
											...
											<td class="p-1 font-xxss">{{ (parseFloat(it.unit_cost) + parseFloat(it.shipping_cost)) *  it.bo_quantity}}</td>
										</tr>
										<tr class="">
										...
										<tr v-if="it.eval_flag==0">
											<td colspan="2" class="bg-yellow-100 py-2 " align="center">
												<span class="flex justid space-x-2 w-10 ">
													<label class="text-xs font-bold uppercase p-1 m-0">Accept/Reject:</label>
														<input type="checkbox" :id="'tickbox'+it.id"  class="form-control tickbox" v-model="checkbox[it.id]" :true-value="it.id" false-value="0" v-if="it.eval_flag==0" @click="tickBox()" @change="tickBox()">
												</span>
											</td>
											<td colspan="10" class="bg-yellow-100 py-2">
												<div class="flex space-x-5 p-0 h-6 pl-5">
													...
													<input type="hidden" class="rec_qty" :id="'rec_qty'+it.id" :value="it.bo_quantity">
												</div>
											</td>
										</tr>
