...
import{ onMounted, ref } from "vue"
	...
	const options = {
		...
		buttons: [
			{
				extend: 'copy',
                title: 'User Acceptance (Completed)',
				exportOptions: {
	...
	const getAllReceive = async (page = 1) => {
		...
		let response = await axios.get(`/api/get_all_useracceptance_accepted?page=${page}`);
		head.value=response.data.receivearray
	}
			...
			<div class="card mb-3">	
				<div class="flex justify-between content-center">
					<div class="flex justify-start space-x-3 ">
						...
						<div>
							<h6 class="m-0 pt-1 font-bold uppercase">User Acceptance <span class="text-sm text-green-600">(Completed)</span></h6>
						</div>
					</div>	
					<div class="pt-1">	
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb adminx-page-breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">User Acceptance (Completed)</li>
							</ol>
						</nav>
					</div>
				</div>
						...
						<div class="table-responsive-md">
							...
							<DataTable :data="head"  :options="options" class="table table-actions table-bordesred table-hover mb-0" width="100%">
								...
								<template #column-7="props">
									<span class="text-white btn btn-xs py-0 !text-xs bg-gray-400 rounded mr-1" v-for="pr in props.rowData.receive_details ">{{ pr.pr_no + ' ' }}</span>
								</template>
								<template #column-8="props">
									<a @click="showTransaction(props.rowData.id)" class="text-white btn btn-xs bg-yellow-500 btn-rounded" v-if="props.rowData.from=='Receive'">
