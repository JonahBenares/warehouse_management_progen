			...
			<div class="card mb-3" id="print_card">	
				<div class="flex justify-between content-center">
					<div class="flex justify-start space-x-3 ">
						...
						<div>
							<h6 class="m-0 pt-1 font-bold uppercase">User Acceptance</h6>
						</div>
					</div>	
					<div class="pt-1">	
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb adminx-page-breadcrumb">
								...
								<li class="breadcrumb-item"><a href="/user_acceptance/pending">User Acceptance</a></li>
				...
				<div class="col-lg-12">
					<div class="flex justify-center">
						<page size="A4" id="printable" class="p-2 ">
							<div class="z-10">
								<printheader>
									USER ACCEPTANCE FORM <p class="text-xs leading-none m-0 font-bold text-green-200"></p>
								</printheader>
								...
								<div  v-for="(det, index) in details" class="border-2 p-2 border-b-0 mt-2">
									...
									<table width="100%" class="table-bordered">
										<tr>
											<td class="text-xs px-1" width="2%" align="center">#</td>
											<td class="text-xs px-1" width="5%" align="center">Acc Qty</td>
											<td class="text-xs px-1" width="5%" align="center">Rej Qty</td>
											<td class="text-xs px-1" width="2%" align="center">U/M</td>
											<td class="text-xs px-1" width="10%" align="center">Part No</td>
											<td class="text-xs px-1" width="20%" align="center">Item Description</td>
											<td class="text-xs px-1" width="20%" align="center">Supplier</td>
											<td class="text-xs px-1" width="10%" align="center">Catalog No.</td>
											<td class="text-xs px-1" width="10%" align="center">Brand</td>
											<td class="text-xs px-1" width="10%" align="center">Color</td>
											<td class="text-xs px-1" width="10%" align="center">Size</td>
											<td class="text-xs px-1" width="10%" align="center">Item Status</td>
											<td class="text-xs px-1" width="10%" align="center">Shipping/U & Other Cost</td>
											<td class="text-xs px-1" width="5%" align="center">Cost</td>
											<!-- <td class="text-xs px-1" width="5%" align="center">Shipping Fee</td> -->
											<td class="text-xs px-1" width="10%" align="center">Total Cost</td>
										</tr>
										<span hidden>{{ qty=0 }} </span>
										<span hidden>{{ x=1 }} </span>
										<tr v-for="(it,index) in det.receive_items.items">
											<span hidden>{{ qty+=it.rec_quantity*(parseFloat(it.unit_cost) + parseFloat(it.shipping_cost)) }}</span>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="center">{{ x }}</td>

											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="center">{{it.rec_quantity}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 !border-2 !border-red-400 font-bold py-1 bg-red-300' : '' : 'text-xs align-top px-1 py-1'" align="center">{{it.rejected_qty}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 !border-2 !border-red-400 font-bold py-1 bg-red-300' : '' : 'text-xs align-top px-1 py-1'" align="center">{{it.uom}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 !border-2 !border-red-400 font-bold py-1 bg-red-300' : '' : 'text-xs align-top px-1 py-1'" align="center">{{it.pn_no}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 !border-2 !border-red-400 font-bold py-1 bg-red-300' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.item_description}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.supplier_name}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.catalog_no}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.brand}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.color}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.size}}</td>
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="left">{{it.item_status}}</td>
								...
								<div class="flex justify-between my-1">
									<div class="">
										...
										<p class="text-xs leading-none m-0">Warehouse Form: User Acceptance Form (Effective June 2018)</p>
									</div>
									<div class="mt-2 ">
										<p class="text-xs leading-none m-0">*Warehouse Copy</p>
										<p class="text-xs leading-none m-0 text-right font-bold text-green-200"></p>
									</div>
								</div>
							</div>
						</page>
