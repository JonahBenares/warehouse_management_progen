...
const warehouseForm = async () => {
	let response = await axios.get("/api/create_warehouse");
...
const onSave = () => {
	...
	axios.post("/api/add_warehouse",formData).then(function () {
		success.value='You have successfully added new warehouse!'
		//form = ref([])
					...
					<div class="pt-1">	
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb adminx-page-breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">Add New Warehouse</li>
							</ol>
						</nav>
					</div>
				</div>
						...
						<div class="row">
							<div class="col-lg-12">
								<div class="form-group">
									...
									<input type="text" class="form-control border" v-model="form.warehouse_name" required>
								</div>										
							</div>
						</div>
