<script setup>
	...
	const getEditRack = async () => {
		let response = await axios.get(`/api/edit_rack/${props.id}`);
        form.value=response.data.racks
	}
	
	const onEdit = (id) => {
		const formData=new FormData()
		formData.append('rack_name',form.value.rack_name)
		axios.post(`/api/update_rack/`+id, formData).then(function () {
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="form-group">
										...
										<input type="text" class="form-control border" v-model="form.rack_name">
									</div>										
								</div>
							</div>
