...
const getRack = async (page = 1) => {
	const response = await fetch(`/api/get_all_rack?page=${page}&filter=${searchRack.value}`);
    racks.value = await response.json();
}

const onEdit= (id) => {
	router.push('/rack/edit/'+id)
}

const search = async () => {
	let response = await fetch('/api/search_rack?filter='+searchRack.value);
    racks.value = await response.json();
}
								...
								<div class="flex justify-between pb-2 mt-2 mb-2">
									<div class="flex justify-between">
										<div class="input-group border rounded-xl w-80">
											...
											<input type="text" class="form-control border-0" id="search" placeholder="Type to search..." @keyup="getRack()" v-model="searchRack">
										</div>
									</div>
									<a href="/rack/new" class="btn btn-sm btn-primary btn-rounded">
								...
								<table class="table table-actions table-hover mb-0">
									...
									<tbody>
										<tr v-for="rack in racks.data" :key="rack.id" >
											...
											<td class="p-1">
												<button @click="onEdit(rack.id)" class="btn btn-xs btn-info btn-rounded text-white">
								...
								<div class="flex justify-end p-2 border-t">
									<nav aria-label="Page navigation example">
										<TailwindPagination
											:data="racks"
											:limit="1"
											@pagination-change-page="getRack"
										/>
									</nav>
								</div>
							</div>
