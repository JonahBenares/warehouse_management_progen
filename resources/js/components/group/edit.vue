<script setup>
	...
	import { CheckCircleIcon, ExclamationCircleIcon, PlusIcon, MagnifyingGlassIcon, ChevronLeftIcon, ChevronRightIcon, ArrowUturnLeftIcon } from '@heroicons/vue/24/solid'
    ...
    const getGroup = async () => {
        let response = await axios.get(`/api/edit_group/${props.id}`)
       form.value = response.data.groups
    }

	const onEdit = (id) => {
		const formData= new FormData()
		formData.append('group_name', form.value.group_name)
		axios.post(`/api/update_group/${form.value.id}`,formData).then(function () {
						...
						<div class="row">
							<div class="col-lg-12">
								<div class="form-group">
									...
									<input type="text" class="form-control border" v-model="form.group_name">
								</div>										
							</div>
						</div>
