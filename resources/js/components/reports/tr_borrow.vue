<script setup>
    ...
    const generateReport = () => {
        ...
        formData.append('borrowed_from', form.value.borrowed_from)
        formData.append('borrowed_by', form.value.borrowed_by)
        ...
        axios.post("/api/all_borrowed_transactions",formData).then(function (response) {
            rows.value=response.data.borrow_data;
    ...
    const getPRfrom = async() => {
		let response = await axios.get("/api/pr_list");
		prfrom.value = response.data.pr;
    }
            ...
            <div class="card mb-3">	
                <div class="flex justify-between content-center">
                    <div class="flex justify-start space-x-3 ">
                        ...
                        <div>
                            <h6 class="m-0 pt-1 font-bold uppercase">Borrow Report</h6>
                        </div>
                    </div>	
                    <div class="pt-1">	
                        <nav aria-label="breadcrumb" role="navigation">
                            <ol class="breadcrumb adminx-page-breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Borrow Report</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                        ...
                        <div class="pt-3 px-4">
                            <div class="row">
                                ...
                                <div class="col-lg-2 px-1">
                                    <div class="form-group">
                                        <div class="flex justify-start ">
                                            <span class="text-xs text-gray-500 leading-none uppercase">Borrow from:</span>
                                        </div>
                                        <span class="text-lg uppercase text-gray-700 w-full leading-none">
                                            <select class="form-control border my-1" v-model="form.borrowed_from">
                                                <option value="">Select PR</option>
                                                <option v-for="pa in prfrom" v-bind:key="pa.pr_no" >{{ pa.pr_no }}</option>
                                            </select>
                                        </span>
                                    </div>										
                                </div>
                                <div class="col-lg-2 px-1">
                                    <div class="form-group">
                                        <div class="flex justify-start ">
                                            <span class="text-xs text-gray-500 leading-none uppercase">Borrow by:</span>
                                        </div>
                                        <span class="text-lg uppercase text-gray-700 w-full leading-none">
                                            <select class="form-control border my-1" v-model="form.borrowed_by">
                                                <option value="">Select PR</option>
                                                <option v-for="p in prfrom" v-bind:key="p.pr_no" >{{ p.pr_no }}</option>
                                            </select>
                                        </span>
                                    </div>										
                                </div>
                            ...
                            <div class="row" style="display: none;" id="showExport">
                                <div class="col-lg-12 px-1">
                                    <div class="flex justify-end mb-3">
                                        <a :href="'/export-borrow/'+((form.from_date!='') ? form.from_date : 'null')+'/'+((form.to_date!='') ? form.to_date : 'null')+'/'+((form.item_name!='') ? form.item_name : '0')+'/'+((form.borrowed_from!='') ? form.borrowed_from : 'null')+'/'+((form.borrowed_by!='') ? form.borrowed_by : 'null')+'/'+((form.category!='') ? form.category : '0')+'/'+((form.subcategory!='') ? form.subcategory : '0')+'/'+((form.department!='') ? form.department : '0')+'/'+((form.enduse!='') ? form.enduse : '0')+'/'+((form.purpose!='') ? form.purpose : '0')" class="btn btn-sm btn-success">
                                        ...
                                        <DataTable :data="rows" :options="options" class="display text-xs table-bordered nowrap" width="280%">
                                            <thead> 
                                                <tr class="bg-gray-200 font-bold sticky top-0 z-50">
                                                    <th class="px-2 py-1 text-center" width="3%">Borrow Date</th>
                                                    <th class="px-2 py-1" width="5%">Borrow from</th>
                                                    <th class="px-2 py-1" width="5%">Borrow by</th>
                                                    <th class="px-2 py-1" width="5%">MBR No.</th>
                                                    <th class="px-2 py-1">Part No.</th>
                                                    <th class="px-2 py-1 sticky left-0 top-0 bg-gray-200 " width="10%">Item Description</th>
                                                    <th class="px-2 py-1" width="4%">Total Qty Borrow</th>
                                            ...
                                            <!-- <tr class="hover:bg-yellow-100 bg-slate" v-for="r in rows">
                                                <td class="px-2 py-1 text-center">{{r.borrow_date}}</td>
                                                <td class="px-2 py-1">{{ r.borrowed_from }}</td>
                                                <td class="px-2 py-1">{{ r.borrowed_by }}</td>
                                                <td class="px-2 py-1">{{r.mbr_no}}</td>
                                                <td class="px-2 py-1">{{ r.pn_no }}</td>
                                                <td class="px-2 py-1 sticky left-0 bg-white">{{ r.item_description }}</td>
                                                <td class="px-2 py-1">{{ r.quantity }}</td>
                                                <td class="px-2 py-1">{{ r.uom }}</td>
                                                <td class="px-2 py-1">{{ r.unit_cost }}</td>
                                                <td class="px-2 py-1">{{ r.quantity * r.unit_cost }}</td>
                                                <td class="px-2 py-1">{{ r.supplier_name }}</td>
                                                <td class="px-2 py-1">{{ r.department_name }}</td>
                                                <td class="px-2 py-1">{{ r.enduse_name }}</td>
                                                <td class="px-2 py-1">{{ r.purpose_name }}</td>
                                            </tr>
                                        </table> -->
                                    </div>
                                </div>
