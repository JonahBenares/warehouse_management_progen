...
class ReceiveExport implements FromQuery, WithMapping, ShouldAutoSize, WithHeadings, WithEvents, WithTitle, WithCustomStartCell, WithDrawings, WithStrictNullComparison
{
    /**
    ...
    */
    ...
    public function query()
    {
        ...
        return $query;
        // ReceiveItems::with(['receive_head','receive_details','items'])->when((!empty($from) && !empty($to)), function ($q) use ($from, $to) {
        //     return $q->whereHas('receive_head', function ($query) use ($from,$to) {
        //         $query->whereBetween('receive_date', [$from, $to]);
        //     });
        // })->when((!empty($item)), function ($q) use ($item) {
        //     return $q->where('item_id',$item);
        // })->when((!empty($pr_no)), function ($q) use ($pr_no) {
        //     return $q->whereHas('receive_details', function ($query) use ($pr_no) {
        //         $query->where('pr_no', $pr_no);
        //     });
        // })->when((!empty($category)), function ($q) use ($category) {
        //     return $q->whereHas('items', function ($query) use ($category) {
        //         $query->where('item_category_id', $category);
        //     });
        // })->when((!empty($subcategory)), function ($q) use ($subcategory) {
        //     $q->whereHas('items', function ($query) use ($subcategory) {
        //         $query->where('item_sub_category_id', $subcategory);
        //     });
        // })->when((!empty($department)), function ($q) use ($department) {
        //     $q->whereHas('receive_details', function ($query) use ($department) {
        //         $query->where('department_id', $department);
        //     });   
        // })->when((!empty($enduse)), function ($q) use ($enduse) {
        //     $q->whereHas('receive_details', function ($query) use ($enduse) {
        //         $query->where('enduse_id', $enduse);
        //     });    
        // })->when((!empty($purpose)), function ($q) use ($purpose) {
        //     $q->whereHas('receive_details', function ($query) use ($purpose) {
        //         $query->where('purpose_id', $purpose);
        //     });    
        // })->query();
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) { 
                ...
                for($x=1;$x<=3;$x++){
                    $event->sheet->getStyle('T'.$x)->applyFromArray([
                        'borders' => [
                            'right' => [
                                'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                            ]
                ...
                $event->sheet->getStyle('C3:T3')->applyFromArray([
                    'borders' => [
                        'bottom' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                        ]
                ...
                for($i=9;$i<=$totalRows;$i++){
                    ...
                    $event->sheet->getStyle('I'.$i.':M'.$i)->applyFromArray([
                        'alignment' => [
                             'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                         ]
                     ]);
                ...
                $event->sheet->getStyle('A9:T9')->getAlignment()->setHorizontal('center');
                ...
                $event->sheet->setCellValue('H2', 'SUMMARY OF RECEIVED MATERIALS');
